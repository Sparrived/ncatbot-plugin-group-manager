...
[![Version](https://img.shields.io/badge/version-1.3.1--post1-orange.svg)](https://github.com/Sparrived/ncatbot-plugin-group-manager/releases)
...
| `essence_list_count`           | `int`       | `5`             | 每次列出多少条精华消息。                                                 |
...
| 指令                                  | 参数                                         | 说明                                                       | 示例                                                                |
| ------------------------------------- | -------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------- |
| `/gm essence random`                  | 无                                           | 随机发送一条群精华消息                                     | `/gm essence random`                                                |
| `/gm essence list [page] [-a\|--all]` | `page`：页码，默认 1<br>`-a/--all`：显示全部 | 列出群内所有群精华消息，支持分页显示                       | `/gm essence list`<br>`/gm essence list 2`<br>`/gm essence list -a` |
| `/gm essence add [mid]`               | `mid`：消息ID（可选）                        | 添加群精华消息，可回复消息或提供消息ID                     | `/gm essence add`<br>`/gm essence add 123456`                       |
| `/gm essence remove [mid]`            | `mid`：消息ID（可选）                        | 删除群精华消息，可回复消息、提供消息ID或自动删除上次随机的 | `/gm essence remove`<br>`/gm essence remove 123456`                 |
...
### 精华消息管理
- **随机播送**: 随机从群精华中选择一条消息发送，展示历史精彩瞬间
- **列表查看**: 支持分页查看所有精华消息，包括消息ID、发送者、时间和内容
- **添加精华**: 管理员可通过回复消息或提供消息ID来添加精华
- **删除精华**: 管理员可通过回复消息、提供消息ID或自动删除上次随机的精华消息
- **智能记忆**: 自动记录上次随机发送的精华消息ID，方便快速删除
...
**Q: 精华消息功能如何使用？**
- 使用 `/gm essence random` 随机播送一条精华消息
- 使用 `/gm essence list` 查看所有精华消息（支持分页）
- 回复要添加的消息并发送 `/gm essence add` 可添加精华
- 回复要删除的消息并发送 `/gm essence remove` 可删除精华
- 或者在随机播送后直接使用 `/gm essence remove` 删除刚播送的精华
